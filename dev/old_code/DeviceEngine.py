...
class DeviceEngine(Engine):
    """
    DeviceEngine class inherited from Engine class. Handles the actual device-related processing tasks.
    Attributes are declared by passing appropriate class objects to initialization.

    Attr:

    Class Attributes:
        headers (Metadata, optional): The project headers. Instance of Metadata class or dict.
        settings (list, optional): The list of settings. Instance or list of instances of ProcessingMethod class.
        analyses (list, optional): The list of analyses. Instance or list of instances of DeviceAnalyses class.
    ...
    Instance Attributes (Unique to each DeviceEngine instance/object):
        _source (raw_str, optional): Path to source directory of data/Analyses.
                                     Can be an individual Analyses or parent directory of multiple analyses.
    ...
    Methods: (specified are methods only belonging to child class or modified to fit child class specifications. For superclass methods, see Engine)
        ...
        get_analyses(self, analyses(str/int/list(str/int))):
                ...
                Returns only a list of DeviceAnalyses Objects.
        ...
        get_seasonal_components(self, data(dict)) :
                Breaks (pressure) curves down into their respective trend, seasonal and residual components for further Analyses.
    ...
    def find_analyses(self):
        ...
        """
        # All instances of DeviceEngine will use the same Datetime methods for Analyses.
        ...
        while dir_stack:
            ...
            for file in os.listdir(current_folder):
                ...
                pathname, extension = os.path.splitext(
            ...
            # if not a superfolder, check whether we are not already within a .D folder
                ...
                if not runs_list:
                    ...
                    for d in runs_list:
                        ...
                        target_files = [
                            f for f in os.listdir(current_run_folder) if filetype in f
                        ]
                        ...
                        with open(run_file, encoding=character_encoding) as f:
                            ...
                            for line in f:
                                ...
                                if "Run" in line:
                                    ...
                                    start_date_string = datetime_pattern.search(
                                        line
                                    ).group()
                                    ...
                                    found_postrun = 1
                                    end_date_string = datetime_pattern.search(
                                        line
                                    ).group()
                            ...
                            print(
                                "Times started(in event of fault) : "
                                + str(times_started)
                            )
                        ...
                        try:
                            ...
                            # Batch idle time for first Analyses duplicated for every Analyses of that (untrimmed) method
                            ...
                            if blank_identifier == 1:
                                ...
                                curve_header = "Sample - " + (
                                    str(pressure_suffix).zfill(suffix_digits)
                                    + run_suffix
                                )
                                ...
                                # add sample run headers to samples list, keep trailing pressure suffix for future Analyses
                            ...
                            for col in pressure_file.select_dtypes(
                                include=["object"]
                                ...
                                if pressure_file[col].str.contains(",").any():
                                    ...
                                    pressure_file[col] = pd.to_numeric(
                                        pressure_file[col], errors="coerce"
                                    )
                            ...
                            runtime_percent_error = (
                                abs(runtime_delta) / curve_runtime
                            ) * 100
                            ...
                            if len(curves_list) > 1:
                                ...
                                curves_list[0] = pd.merge(
                                    curves_list[0], curves_list[-1], on="Time"
                                )

                        if curve_header:
                            # if pressure curve exists for current Analyses, mention in Analyses key
                            # experiment date and method ID as identifier for Analyses name. Name is name from Analyses object
                            Analyses_name = f"Device Pressure Analyses - {method_suffix}| Start time: {start_date_string}"
                            Analyses_type = "pressure"
                            ...
                            Analyses_name = f"Analyses - {method_suffix} | Start time: {start_date_string}"

                        # each 'Device Pressure Analyses' dict like below refers to an individual run within a set of consecutive runs for a particular method on a particular date.
                        # This will be updated during operations concerning the particular runs such as feature extraction.
                        # the larger dict holding these individual dicts is the data of a single DeviceAnalyses object.
                        Analyses_data = {
                        ...
                        for index in range(component_number):
                            Analyses_data.update(
                                {f"Component {index+1}": comp_ids[index]}
                            )

                        data_keyList = list(Analyses_data.keys())

                        # add every encountered Analyses to device history
                        self._history.update({Analyses_name: Analyses_data})
                        ...
                        # create new DeviceAnalyses object.
                        new_object = DeviceAnalyses(
                            name=Analyses_name,
                            data=Analyses_data,
                            Analyses_type=Analyses_type,
                            class_label=class_label,
                            key_list=data_keyList,
                        )

                        # list of analyses populated with individual Analyses objects
                ...
                if method_suffix not in method_ids:
                    ...
                    self._method_ids.update(
                        {method_suffix: (first_run_of_batch, last_run_of_batch)}
                    )
    ...
    def get_features(self, data, features_list, weighted):
        ...
        # update each Device Pressure Analyses with its features
        curve = data["Curve"]
        sample = data["Sample"]
        ...
        if weighted == True:
            weighted_curve_features = (
                curve.iloc[:, 1].agg("pct_change") * 100
            ).replace([np.inf, -np.inf], np.nan)
            weighted_curve_features = (weighted_curve_features.fillna(0)).agg(
                features_list
            )
            weighted_curve_features.index = [
                f"{i}_percent_change" for i in weighted_curve_features.index
            ]
        ...
        run_features = pd.DataFrame(
            [runtime, runtime_delta, runtime_percent_error, runtype, bpos, idle_time],
    ...
    def bin_frequencies(self, data):
        ...
        Args: (self : DeviceEngine object, data(DeviceAnalyses.data) : data entry of an individual Analyses object)
        ...
        """create bin edges"""
        seasonal_bin_edges = np.linspace(
            min(seasonal_freq_bins), max(seasonal_freq_bins), num=num_seasonal_bins
        )
        #
        noise_bin_edges = np.linspace(
            min(noise_freq_bins), max(noise_freq_bins), num=num_noise_bins
        )

        """create bins based on preset edges and determine grouping of data"""
        seasonal_bin_indices = np.digitize(
            seasonal_freqs, seasonal_bin_edges, right=True
        )
        #
        ...
        combined_Analyses_mags = pd.concat(
            [
                mean_binned_seasonal_magnitudes,
                sum_binned_seasonal_magnitudes,
                mean_binned_noise_magnitudes,
                sum_binned_noise_magnitudes,
            ],
            ignore_index=True,
        )

        # create a DataFrame with a single column
        bins_df = combined_Analyses_mags.to_frame(name=data["Sample"])
        new_feat_names = []
    ...
    def scale_features(self, prepared_data, type):
        ...
        for ana in prepared_data:
            # each ana is an Analyses object
            data = ana.data
            features_df = data["Features"]
    ...
    def get_seasonal_components(self, data, period):
        ...
        Now each 'Device Pressure Analyses' dict has been modified with additional seasonal component data for each individual curve
        ...
        decomp = seasonal_decompose(
            pd.to_numeric(sample_curve[sample_name]),
            model="additive",
            period=period,
        ...
        trend, seasonal, residual = (
            pd.Series(decomp.trend, name=sample_name),
            pd.Series(decomp.seasonal, name=sample_name),
            pd.Series(decomp.resid, name=sample_name),
        )

        data.update({"Trend": trend, "Seasonal": seasonal, "Residual": residual})
    ...
    def make_fourier_transform(self, data):
        ...
        # if 'Seasonal' in data[Analyses_key]:
        seasonal = data["Seasonal"]
    ...
    def get_results(self, results=None, scaled=False):
        """
        Retrieves the results from Engine.
        ...
        if isinstance(results, str):
            ...
            for result in results:
                if isinstance(result, str):
                    ...
                    key_list = list(self._results.keys())
                    results_out.update(
                        {key_list[result]: self._results[key_list[result]]}
                    )
        ...
        if scaled == True:
            for resname in found_results:
                if "_scaled" in resname or "Device Pressure Analyses" not in resname:
        ...
        # if scaled False or unspecified, return unscaled data
            ...
            for resname in found_results:
                if "_scaled" not in resname or "Device Pressure Analyses" in resname:
                    found_dict.update({resname: result_dict[resname]})
    ...
    def trim_method_name(self, method_name=None):
        """
        Method to trim Analyses/Method names for comparison and grouping.
        ...
        if (method_name[0] != "_" and method_name[-1] != "_") and len(
            method_name
        ) <= 10:
    ...
    def group_analyses(self, data_list, group_by):
        ...
        # helper functions
        def initialize_group(Analyses, group_by):
            # Initialize a new group with the first Analyses data
            method_name = Analyses.data["Method"]
            if group_by == "method":
                method_name = self.trim_method_name(Analyses.data["Method"])

            return {"Method": method_name, "Data": Analyses.data}

        def can_merge(current_group, new_Analyses, group_by):
            # Logic to determine if two analyses can be merged
            if group_by == "method":
                return self.trim_method_name(
                    current_group["Method"]
                ) == self.trim_method_name(new_Analyses.data["Method"])
            else:
                return current_group["Method"] == new_Analyses.data["Method"]

        def merge_groups(current_group, new_Analyses):
            # Logic to merge the data from new_Analyses into current_group
            new_data = new_Analyses.data
            ...
            for data_item in [
                ...
                if data_item == "Curve":
                    df = pd.merge(
                        current_data[data_item], new_data[data_item], on="Time"
                    )
                    ...
                    if data_item in list(current_data.keys()):
                        ...
                        merged_data.update(
                            {
                                data_item: pd.concat(
                                    [current_data[data_item], new_data[data_item]],
                                    axis=1,
                                )
                            }
                        )
        ...
        if isinstance(data_list, dict):
            for key, value in data_list.items():
                if "_scaled" in key or "Device Pressure Analyses" not in key:
                    print(f"{key} is already prepared!")
                    scaled_anas.append(DeviceAnalyses(name=key, data=value))
                else:
                    anas_to_plot.append(DeviceAnalyses(name=key, data=value))
        ...
        for ana in anas_to_plot[1:]:
            if can_merge(current_group, ana, group_by):
                ...
                if current_method not in names_list:
                    objects_list.append(
                        DeviceAnalyses(name=current_method, data=current_group["Data"])
                    )
                else:
                    # match the index of the Analyses that has the same name as the current one
                    ...
                    grouped = self.group_analyses(
                        [
                            objects_list[unikey_index],
                            DeviceAnalyses(
                                name=current_method, data=current_group["Data"]
                            ),
                        ],
                        group_by,
                    )
        ...
        if group_by != "method":
            for ana in objects_list:
                if (
                    ana.data["Method"][0] != "_" and ana.data["Method"][-1] != "_"
                ) and len(ana.data["Method"]) <= 10:
                    del ana
        return objects_list

    def plot_analyses(self, analyses=None, interactive=True, group_by="method"):
        """
        Plots each Analyses dataframe by calling plot() function of respective DeviceAnalyses objects

        """
        # retrieve list of Analyses objects based on user input
    ...
    def get_feature_matrix(self, results=None):
        ...
        # migrate prepared and scaled features matrix as MachineLearningAnalyses data to MachineLearningEngine
        ml_objects = []
        x_values = list(new_df.columns)
        samples = list(new_df.index)
        for i in range(len(new_df)):
            name = samples[i]
            data = {"x": x_values, "y": new_df.iloc[i]}
            ml_objects.append(MachineLearningAnalyses(name=name, data=data))

        # create MLEngine objects for each method-grouped set of results like 'Pac' or 'Doc'

        ml_engine = MachineLearningEngine(
            headers={"name": f"ML{results}", "author": "Sandeep H."},
            analyses=ml_objects,
        )
