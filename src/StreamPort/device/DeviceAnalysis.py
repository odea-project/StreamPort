from src.StreamPort.core.Engine import Analyses
...
class DeviceAnalyses(Analyses):
    """
    Represents a DeviceAnalyses object, inherited from Analyses Class.

    Class Attributes:
        name (str): The name of the Analyses. Uniquely identified using the method name and the date of creation.
        replicate (str): The name of the replicate.
        blank (str): The name of the blank.
        data (dict/list): The data of the Analyses, which is a dict or list of one dimension numpy arrays or dicts or lists.
        ****Set data as a list of Analyses child objects like pressure Analyses, actuals

    Instance Attributes:
        _Analyses_type (str/list(str), optional): Marker(s) to specify the type of data the current Analyses is related to (pressure, temperature, actuals,..)

        ***Note*** : _Analyses_type must have same size as data. Will influence future functionality of class methods.
        ...
        _key_list (list(str)): list of Analyses data keys in order of appearance. Maybe different for other data types.

    Methods: (specified are methods only belonging to child class. For superclass methods, see Analyses)

        validate (self, _Analyses_type) : Validates the Analyses object while allowing for flexibility in handling varying datatypes for each DeviceAnalyses instance.

        plot (self, analyses(DataFrame/list(DataFrame))) : Plots the selected (pressure) curves.

        set_class_label (self, class_label(str)) : Sets class label of Analyses object using assigned label from unsupervised learning.
    ...
    def __init__(
        ...
        self._Analyses_type = (
            str(Analyses_type)
            if not isinstance(Analyses_type, type(None))
            else "Unknown"
        )
        self._key_list = (
            key_list if not isinstance(key_list, type(None)) else list(self.data.keys())
        )
    ...
    def __str__(self):
        """
        Returns a string representation of the Analyses object.
            ...
            data_str = "\n".join(
                [
                    (
                        ...
                        else f"{key} : (size {len(str(self.data[key]))})"
                    )
    ...
    def plot(
        ...
        ***Note***features, decomp and transform may only be plotted one at a time.
        ...
        if features == True:
            ...
            if (
                scaled == True
                and "Device Pressure Analyses" not in self.name
                or "_scaled" in self.name
            ):
        ...
        colors_list = [
            f"hsv({i*360/num_labels}, 100%, 100%)" for i in range(num_labels)
        ]
        ...
        for sample in samples:
            if features == True:
                ...
                curves.update(
                    {
                        sample: (
                            data["Raw curve frequencies"][sample],
                ...
                if type == "box":
                    ...
                    fig.add_trace(
                        go.Box(
                            ...
                            visible=True,
                            name=labelname,
                            marker=dict(color=colors_list[index], opacity=0.4),
                            hovertext=sample_name,
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            mode="lines",
                            marker=dict(
                                size=5, color=colors_list[index], line=dict(width=0)
                            ),
                            text=sample_name,
                            legendgroup=f"group{index}",
                        ),
    ...
    def set_class_label(self, class_label=None):
        ...
        """
        if not isinstance(class_label, type(None)) and not "001-blank" in self.data.get(
            "Sample", []
        ):
            ...
            if (
                self.data != {}
                and "Sample" in self.data
                and "001-blank" in self.data["Sample"]
            ):
...
class DevicePressureAnalyses(DeviceAnalyses):
    ...
    def __init__(
        ...
        super().__init__(
            name, replicate, blank, data, Analyses_type, class_label, key_list
        )
        self._Analyses_type = Analyses_type
        self._key_list = (
            key_list if not isinstance(key_list, type(None)) else list(self.data.keys())
        )


class DeviceActuals(DeviceAnalyses):
...
class DeviceMetadata(DeviceAnalyses):
