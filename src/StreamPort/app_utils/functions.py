...
# Feature Extraction
def extract_features(analyses, processor):
    analyses = processor.run(analyses) # Fit debug values show a 2-class spread of amplitudes.
    return analyses

#Train Set Selection
def select_train_set_pc(pc, method='SAA_411_Pac.M'):
    date_threshold_min = datetime(2021, 8, 19)
    indices = pc.get_method_indices(method)
    """
    Train Set
    """
    train_indices = []
...
def select_train_set_ms(analyses):
...
# Scaling
def scale_data(analyses, train_indices, scaler_type: str=None):
...
# Testing
def test_sample(ml, analyses, test_index, threshold="auto", n_tests=None):
    test_data = analyses.get_features(test_index)
    test_metadata = analyses.get_metadata(test_index)
